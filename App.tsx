...
const POLLINATIONS_TEXT_FALLBACK_API = "https://text.pollinations.ai";
...
const App: React.FC = () => {
  ...
  const handleAPIError = (e: any) => {
    const msg = String(e);
    console.error("Free generation provider error:", msg);
  };

  const generateText = async (prompt: string): Promise<string> => {
    try {
      const res = await fetch(POLLINATIONS_TEXT_API, {
        method: 'POST',
      ...
      if (!res.ok) throw new Error(`Pollinations text API failed: ${res.status}`);
      const data = await res.json();
      const content = data?.choices?.[0]?.message?.content;
      if (typeof content === 'string' && content.trim()) return content;
      throw new Error('Pollinations text API returned an empty response');
    } catch (primaryError) {
      console.warn('Falling back to simple Pollinations text endpoint', primaryError);
      const fallback = await fetch(`${POLLINATIONS_TEXT_FALLBACK_API}/${encodeURIComponent(prompt)}`);
      if (!fallback.ok) throw new Error(`Pollinations text fallback failed: ${fallback.status}`);
      return fallback.text();
    }
  };

  const generateImageDataUrl = async (prompt: string, width = 1024, height = 1536): Promise<string> => {
    ...
    const url = `${POLLINATIONS_IMAGE_API}/${encodeURIComponent(prompt)}?${params.toString()}`;
    const res = await fetch(url);
    if (!res.ok) throw new Error(`Pollinations image API failed: ${res.status}`);
...
`;
  ...
  const generatePersona = async (desc: string): Promise<Persona> => {
      ...
      try {
          const dataUrl = await generateImageDataUrl(`STYLE: Masterpiece ${style} character sheet, detailed ink, neutral background. FULL BODY. Character: ${desc}`, 1024, 1024);
          return { base64: dataUrlToBase64(dataUrl), desc };
      } catch (e) { 
        handleAPIError(e);
        throw e; 
      }
  ...
  const generateImage = async (beat: Beat, type: ComicFace['type']): Promise<string> => {
    ...
    promptText += `Main character: ${heroRef.current?.desc || 'The Main Hero'}. `;
    if (friendRef.current) promptText += `Co-star: ${friendRef.current.desc}. `;
    
    if (type === 'cover') {
        const langName = LANGUAGES.find(l => l.code === selectedLanguage)?.name || "English";
        promptText += `TYPE: Comic Book Cover. TITLE: "INFINITE HEROES" (OR LOCALIZED TRANSLATION IN ${langName.toUpperCase()}). Main visual: Dynamic action shot of the hero.`;
        ...
        promptText += `INSTRUCTIONS: Keep a consistent comic-book cast across pages.`;
    ...
    try {
        return await generateImageDataUrl(promptText, 1024, 1536);
    } catch (e) { 
        handleAPIError(e);
        return ''; 
    }
